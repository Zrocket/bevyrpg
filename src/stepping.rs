use avian3d::dynamics::solver::schedule;
use bevy::{app::MainScheduleOrder, ecs::schedule::{InternedScheduleLabel, NodeId, ScheduleLabel, Stepping}, prelude::*};
...
const FONT_BOLD: &str = "fonts/FireSans-Bold.ttf";
...
///
/// This system may run multiple times beforer constructing the UI as all of the
/// data may not be avilable on the first run of the systetm. This happens if
/// one of the stepping shcedules has not yet been run.
fn build_ui(
    ...
    let mut always_run: Vec<(
        bevy::ecs::intern::Interned<dyn ScheduleLabel + 'static>,
        NodeId,
    )> = Vec::new();
    ...
    // go through the stepping schedules and construct a list of systetms for
    // each label
...
fn handle_input(keyboard_input: Res<ButtonInput<KeyCode>>, mut stepping: ResMut<bevy::ecs::schedule::Stepping>) {
