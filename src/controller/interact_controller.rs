...
use crate::{Inspectable, InteractEvent, RESOLUTION_HEIGHT, RESOLUTION_WIDTH};
use avian3d::prelude::*;
use avian_pickup::{
...
pub fn manage_inspect(
    ...
    if let Ok((player, ray_hit)) = player.single() {
        ...
        /*for (camera, global_transform) in camera_query.iter() {
            info!("got cam and global transform");
            let center_window = camera.viewport_to_world(global_transform, Vec2 { x: RESOLUTION_WIDTH / 2., y: RESOLUTION_HEIGHT / 2. }).unwrap();
            let camera_position = global_transform.translation();
            let camera_direction = global_transform.forward();
            if let Some(ray_data) = ray_caster.cast_ray(
                center_window.origin,
                ...
                info!("inspect ray casted");
                let hit_point = camera_position + camera_direction * ray_data.distance;
                if let Ok(inspection) = inspection_query.get(ray_data.entity) {
                    info!("ray inspection");
                    for act in inspection.iter() {
                        info!("INTERACT Entity {:?} hit at point {}, from {}",
                            ray_data.entity, hit_point, camera_position
                        );
                        act.inspect(&mut commands, player, ray_data.entity);
                    }
                }
            }
        }*/
